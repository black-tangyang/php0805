...
<div id="main-div">
    <div id="menu-list">
        <ul id="menu-ul">
            <?php if(is_array($menulist)): $i = 0; $__LIST__ = $menulist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i; if(($row["level"]) == "1"): ?><li class="explode" key="02_cat_and_goods" name="menu">
            <?php echo ($row['name']); ?>
                <ul>
                    <?php if(is_array($menulist)): $i = 0; $__LIST__ = $menulist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row2): $mod = ($i % 2 );++$i; if(($row2["parent_id"]) == $row["id"]): ?><li class="menu-item"><a href="<?php echo U($row2['url']);?>" target="main-frame"><?php echo ($row2['name']); ?></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                </ul>
